...
function handleAction(ev) {
	if (ev === 'restart' || ev === 'reload') {
		...
		return dom.callClassMethod(map, 'save')
			...
			.then(function () {
				ui.showIndicator('banip-action', _('Applying configuration & reloading SecBox...'));
				return fs.exec_direct('/etc/init.d/banip', [ev])
					.then(function () {
						ui.hideIndicator('banip-action');
						ui.addNotification(null, E('p', _('SecBox configuration saved and service %sed successfully.').format(ev)), 'info');
					})
					.catch(function (e) {
						ui.hideIndicator('banip-action');
						ui.addNotification(null, E('p', _('Failed to %s SecBox: %s').format(ev, e.message)), 'error');
					});
			});
	} else {
		return fs.exec_direct('/etc/init.d/banip', [ev]);
	...
	render: function (result) {
		...
		*/
		if (!result[3] || result[3].length === 0) {
			ui.addNotification(null, E('p', _('No SecBox config found!')), 'error');
			return;
		}

		let m, s, o;
		const docLink = '<a style="color:#37c;font-weight:bold;" href="https://github.com/openwrt/packages/blob/master/net/banip/files/README.md" target="_blank" rel="noreferrer noopener">' + _('online documentation') + '</a>';
		const mapDescr = _('Configuration of the SecBox package to ban incoming and outgoing IPs via named nftables Sets. For further information please check the %s.').format(docLink);
		m = new form.Map('banip', _('SecBox Settings'), mapDescr);
		...
		*/
		...
		o.rawhtml = true;
		o.default = '<em style="color:#37c;font-weight:bold;">' + _('Changes on this tab need a SecBox service restart to take effect.') + '</em>'
		...
		o = s.taboption('general', widgets.NetworkSelect, 'ban_trigger', _('Startup Trigger Interface'), _('List of available network interfaces to trigger the SecBox start.'));
		o.multiple = true;
		o.nocreate = true;
		o.rmempty = true;

		o = s.taboption('general', form.Value, 'ban_triggerdelay', _('Trigger Delay'), _('Additional trigger delay in seconds before SecBox processing begins.'));
		o.placeholder = '10';
		...
		*/
		...
		o.rawhtml = true;
		o.default = '<em style="color:#37c;font-weight:bold;">' + _('Changes on this tab needs a SecBox service restart to take effect.') + '</em>'
			+ '<hr style="width: 200px; height: 1px;" />';

		o = s.taboption('advanced', form.ListValue, 'ban_nicelimit', _('Nice Level'), _('The selected priority will be used for SecBox background processing.'));
		...
		o = s.taboption('advanced', form.ListValue, 'ban_cores', _('CPU Cores'), _('Limit the cpu cores used by SecBox to save RAM, autodetected by default.'));
		o.value('1');
		o.value('2');
		o.value('4');
		o.value('8');
		...
		o = s.taboption('advanced', form.Value, 'ban_basedir', _('Base Directory'), _('Base working directory while SecBox processing.'));
		o.placeholder = '/tmp';
		o.rmempty = true;

		o = s.taboption('advanced', form.Value, 'ban_backupdir', _('Backup Directory'), _('Target directory for compressed feed backups.'));
		o.placeholder = '/tmp/SecBox-backup';
		o.rmempty = true;

		o = s.taboption('advanced', form.Value, 'ban_reportdir', _('Report Directory'), _('Target directory for SecBox-related report files.'));
		o.placeholder = '/tmp/SecBox-report';
		o.rmempty = true;

		o = s.taboption('advanced', form.Value, 'ban_errordir', _('Error Directory'), _('Target directory for SecBox-related error files.'));
		o.placeholder = '/tmp/SecBox-error';
		o.rmempty = true;
		...
		*/
		...
		o = s.taboption('adv_chain', form.ListValue, 'ban_nftpriority', _('Chain Priority'), _('Set the NFT chain priority within the SecBox table, lower values means higher priority.'));
		o.value('10');
		o.value('0');
		o.value('-100');
		o.value('-150');
		...
		*/
		...
		o = s.taboption('adv_set', form.ListValue, 'ban_nftpolicy', _('Set Policy'), _('Set the NFT policy for SecBox-related Sets.'));
		...
		*/
		...
		o = s.taboption('adv_email', form.Flag, 'ban_mailnotification', _('E-Mail Notification'), _('Receive E-Mail notifications with every SecBox run.'));
		o.rmempty = true;

		o = s.taboption('adv_email', form.Value, 'ban_mailreceiver', _('E-Mail Receiver Address'), _('Receiver address for SecBox notification E-Mails, this information is required to enable E-Mail functionality.'));
		o.placeholder = 'name@example.com';
		o.rmempty = true;

		o = s.taboption('adv_email', form.Value, 'ban_mailsender', _('E-Mail Sender Address'), _('Sender address for SecBox notification E-Mails.'));
		o.placeholder = 'no-reply@SecBox';
		o.rmempty = true;

		o = s.taboption('adv_email', form.Value, 'ban_mailtopic', _('E-Mail Topic'), _('Topic for SecBox notification E-Mails.'));
		o.placeholder = 'SecBox notification';
		o.rmempty = true;

		o = s.taboption('adv_email', form.Value, 'ban_mailprofile', _('E-Mail Profile'), _('Profile used by \'msmtp\' for SecBox notification E-Mails.'));
		...
		*/
		...
		o.rawhtml = true;
		o.default = '<em style="color:#37c;font-weight:bold;">' + _('Changes on this tab needs a SecBox service reload to take effect.') + '</em>'
			+ '<hr style="width: 200px; height: 1px;" />'
		...
		o = s.taboption('feeds', form.Flag, 'ban_autoallowlist', _('Auto Allowlist'), _('Automatically add resolved domains and uplink IPs to the local SecBox allowlist.'));
		o.default = 1;
		o.rmempty = false;
		...
		o = s.taboption('feeds', form.Flag, 'ban_autoblocklist', _('Auto Blocklist'), _('Automatically add resolved domains and suspicious IPs to the local SecBox blocklist.'));
		o.default = 1;
		o.rmempty = false;
